...
impl Invaders {
    ...
    fn update_invader_positions(&mut self, should_move_downwards: bool) {
        if should_move_downwards {
            ...
            for invader in self.army.iter_mut() {
                invader.position.col =
                    ((invader.get_col() as i32) + self.direction as i32) as usize;
            }
        }
    }
    ...
    fn update_invader_look(&mut self) {
        for invader in self.army.iter_mut() {
            let value = self.move_timer.percent_left();
